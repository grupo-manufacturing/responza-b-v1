...
const INSTAGRAM_REACTION_BY_EMOJI: Record<string, string> = {
  '👍': 'like',
  '❤️': 'love',
  '😂': 'haha',
  '😀': 'wow',
...
function instagramReactionPayload(emoji: string): Record<string, string> {
  const named = INSTAGRAM_REACTION_BY_EMOJI[emoji]
  if (named !== undefined) {
    return { reaction: named }
  }

  return { reaction: 'other', emoji }
}

export async function sendInstagramReaction(input: {
  to: string
  messageId: string
  emoji: string | null
  businessAccountId: string
    ...
    input.emoji === null
      ...
      : {
          ...
          payload: {
            ...
            ...instagramReactionPayload(input.emoji),
          },
        }
