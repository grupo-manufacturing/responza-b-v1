...
export async function createEmbedding(text: string): Promise<number[]> {
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn('[ai] OpenAI embedding request failed', {
        status: response.status,
        message: data.error?.message ?? 'Unknown error',
      })
      throw new AppError(502, 'INTERNAL_ERROR', 'Embedding request failed. Please try again.')
    }

    const embedding = data.data?.[0]?.embedding
    if (embedding === undefined || embedding.length === 0) {
      throw new AppError(502, 'INTERNAL_ERROR', 'Embedding request returned empty data')
    }
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new AppError(504, 'INTERNAL_ERROR', 'Embedding request timed out. Please try again.')
    }

    logger.warn('[ai] OpenAI embedding request error', {
      error: error instanceof Error ? error.message : String(error),
    })
    throw new AppError(502, 'INTERNAL_ERROR', 'Embedding request failed. Please try again.')
  } finally {
