export function buildAgentSystemPrompt(contextLines: string[]): string {
  const lines = [
    ...
    '- The answer is fully supported by the business profile fields, fetched page content, and/or relevant catalogue excerpts below.',
    'Set shouldReply to false when:',
    '- The message is a greeting only, vague, or needs human judgment.',
    '- It is about orders, payments, delivery tracking, complaints, refunds, appointments, or custom requests.',
    '- It asks for pricing, discounts, or quotes unless the exact price or offer is explicitly stated in the catalogue excerpts below.',
    '- It requires thread-specific facts not present in the business context.',
    '- A page could not be fetched and the answer is not already in the profile text — do not guess from the URL alone.',
